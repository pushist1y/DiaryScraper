    ...
    public class CloudflareEvader
    {
        ...
        public static WebClient CreateBypassedWebClient(string url)
        {
            ...
            catch (WebException ex) //We usually get this because of a 503 service not available.
            {
                ...
                using (var f = File.CreateText("c:\\work\\test.html"))
                {
                    f.Write(html);
                }

                // html = File.ReadAllText("c:\\work\\testsaved.html");
                ...
                builder = Regex.Replace(builder, @"\w+\s+=\s+document\.createElement.*getElementById\('challenge-form'\);", "var t = {length: 0}   ;", RegexOptions.Multiline);
                builder = Regex.Replace(builder, @"a\.value ", "var res");
                builder = Regex.Replace(builder, @"';.*?'", "");
                builder += "res;";
                ...
                Thread.Sleep(4000); //This sleeping IS requiered or cloudflare will not give you the token!!
                ...
                var newUri = uri_builder.Uri;

                using (var handler = new HttpClientHandler() { CookieContainer = cookie_container, AllowAutoRedirect = false })
                using (var client = new HttpClient(handler))
                {
//                    client.BaseAddress = new Uri($"{newUri.Scheme}://{newUri.Host}");
                    ...
                    // client.DefaultRequestHeaders.TryAddWithoutValidation("Accept-Language", "en-US,en;q=0.8,ru;q=0.6");
                    ...
                    foreach (var v in client.DefaultRequestHeaders.Accept)
                    {
                        if (v.MediaType.Contains("text/html"))
                        {
                            ...
                            field.SetValue(v, "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8"); // avoid MediaType's validation
                            v.CharSet = "";
                        }
                    }
                    ...
                    {
                        var field = v.GetType().GetTypeInfo().GetField("_value", BindingFlags.NonPublic | BindingFlags.Instance);
                        field.SetValue(v, "en-US,en;q=0.8,ru;q=0.6"); // avoid MediaType's validation
                    }

                    foreach (var v in client.DefaultRequestHeaders.Connection)
                    {
                    ...
                    var res = client.GetAsync(newUri).Result;
                }
                ...
                cookie_req.UserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.100 Safari/537.36";


                cookie_req.Headers["Accept"] = "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8";
                cookie_req.Headers["Accept-Encoding"] = "gzip, deflate, br";
                cookie_req.Headers["Accept-Language"] = "en-US,en;q=0.8,ru;q=0.6";
                cookie_req.Headers["Connection"] = "keep-alive";
                cookie_req.Headers["Upgrade-Insecure-Requests"] = "1";


                foreach (var header in cookie_req.Headers)
                {
                    var z = header;
                }
                ...
                var cookie_resp = (HttpWebResponse)cookie_req.GetResponseNoException();
                string responseText;
                using (var reader = new System.IO.StreamReader(cookie_resp.GetResponseStream()))
                {
